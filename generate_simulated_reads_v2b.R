...
logdigest_mat<-function(pairvec,u,v,ncuts,lower,upper,seq) {
    ...
    # compute all combinations of ncuts, and probability of each cleavage pattern
    ldmat=matrix(0,k,k)
    for (ncut in ncuts) {
		  combos <- combn(length(pairvec),ncut)
		  probs <- unlist(apply(combos, 2, function(x) prod(dv[x])))
		  probs <- probs/sum(probs) # normalize so all probabilities for a given ncuts sum to 1
		  # fill in read probability matrix
		  for (i in 1:length(probs)) {
		  	x <- combos[,i]
		  	coords <- rbind(x[-length(x)]+1, x[-1])	# doesnt generate 5' and 3' end fragments
#		  	coords <- if (any(x==k)) {rbind(c(1,x[-length(x)]+1),x)} else { rbind(c(1,x+1),c(x,k))}	# produces all read fragment that come from the given cleavage pattern
		  	ldmat[t(coords)] <- ldmat[t(coords)] + probs[i]
		  }
		}
    ...
    # normalize likelihood to sum to one, then convert to log
...
args <- commandArgs(T)
if (length(args) < 8) {
	cat("USAGE: ./generate_simulated_reads_v2b.R fn u v lower upper ncuts num_reads out_fn\n")
	q()
}
fn = args[1]
...
ncuts <- as.integer(unlist(strsplit(args[6],",")))
read_thres = as.numeric(args[7])
out_fn = args[8]
...
# generate reads
ldmat=logdigest_mat(pairing,u,v,ncuts,lower_size,upper_size,seq)
